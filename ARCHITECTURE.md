...
| `inventoryUI.js` | **Unified inventory grid** (`#inventory`, 2026-07 revamp): ONE responsive auto-fill `.invGrid` of all items with filter chips — type (All/Weapons/Armor/Accessories/Potions, with counts), min-rarity, and ⬆ Upgrades-only (`headlinePower` vs the equipped slot). The unfiltered view renders REAL capacity (`player.functions.inventory()` cells, used + free). Cell badges: `.invPower` headline number, 🔒 `item.locked`, ⚜ `isUnique`. Interactions: click = equip (or sell in 💰 sell mode), Ctrl/Cmd+Click = multi-select (blue ring + "Sell selected (N)" → `itemSell.sellItemsByIds`), Shift+Click = toggle `item.locked` (persists with the item), drag a cell onto the equipped paper-doll (`#equipHtml`, document-level delegation) to equip, RIGHT-click = sell. The 🧪 Potions chip swaps the grid for the hotbar/potion section (`#potionInventory` is ALWAYS in the DOM — `createPotionInventory` writes into it unconditionally). Item detail lives in the **floating tooltip**: a body-level `#floatTip` layer (`showFloatTip`, viewport-clamped, fed by `invTipShow`/`hideFloatTip`; the equipped paper-doll uses it too via `equipTipShow(slot)`) — escapes the overlay panels' `overflow-y:auto` clipping. Stage-3 polish: rarity glow on cell icons (CSS on the rarity class), a NEW badge on `isNew` drops (cleared on first hover), and a "Sell shown (N)" button when a filter is active (`sellShownItems` → `sellItemsByIds` over the visible unlocked set via the shared `passesInvFilter`). Item `<img>`s carry an `onerror` fallback to `questionMark.png`. Equipped items render as a labeled 3-column paper-doll (`.dollGrid`/`.dollCell`/`.dollLabel` in `#equipHtml`: empty slots dimmed `{slot}Empty.png`, filled slots carry the rarity class + click-to-unequip `unequipItem(id,'solo')` + the floating tooltip) | `CreateInventoryWeaponHtml`, `unequipItemLoad`, `EquippedItemsEmpty`, `checkIfEquippedEmpty`, `itemTooltipTest` (reused by shopUI), `showFloatTip` | `CreateInventoryWeaponHtml`, `toggleSellMode`, `invCellClick`, `invTipShow`, `hideFloatTip`, `equipTipShow`, `setInvFilter`, `setInvRarity`, `toggleInvUpgrades`, `sellSelectedItems`, `sellShownItems`, `invDragStart` |
