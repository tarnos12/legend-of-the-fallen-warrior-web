...
| `uniqueSetsUI.js` | "Fallen Legends" set-progress panel (`#uniqueSets`, below the Soul Shop in the Shop tab): pips for boss uniques equipped (0–4), the active set bonus, and a next-tier hint. Reflects the live equip loadout — refreshed on Shop open and via `stats.updateHtml`. | `renderUniqueSets` | `renderUniqueSets` (Shop HUD nav onclick) |
