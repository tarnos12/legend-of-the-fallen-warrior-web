...
| `bestiaryUI.js` | Bestiary panel (`#tab_mainBestiary`): one card per monster grouped by area, driven by persisted `killCount`. Knowledge tiers: 0 = ???, 1+ name/portrait/level, 10+ combat stats, 25+ rewards, 100 = mastered (drop table placeholder). Also shows collectible-card status per monster + per-area set progress/bonus (from `systems/cards.js`). | `BESTIARY_MASTERY_KILLS` | `renderBestiary` (HUD nav onclick) |
