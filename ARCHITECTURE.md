...
| `soulShopUI.js` | Soul Shop section at the top of the Shop tab (`#soulShop`): the player's Boss Soul balance + one card per boss unique (name/slot/area/soul price) with a Buy button (disabled when the area is locked or unaffordable). Buying → `systems/bossSouls.buyBossUnique`. Re-rendered on Shop-tab open, on soul gain, and after a purchase. | `renderSoulShop` | `renderSoulShop` (Shop HUD nav onclick) |
...
| `bossSouls.js` | Boss Souls (2026-07 acquisition rework): `grantBossSouls(monster, quiet, shiny)` (called from `battle.grantKillRewards`) adds `SOUL_DROP` souls to `player.properties.bossSouls` on area-boss kills. `buyBossUnique(bossName)` (window onclick) spends souls to `mintBossUnique` a boss's unique at the player's level, gated on the area being unlocked + enough souls. Souls persist automatically (in `player.properties`, backfilled on load). The HUD `#soulCount` counter is refreshed by `stats.updateHtml` and immediately on gain/spend (`refreshSoulUi`). |
