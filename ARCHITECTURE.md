...
| `public/` | Static assets served as-is (jquery/, vendor/, js/bootstrap, sounds/, css/). `css/theme.css` is the dark-fantasy skin: loaded **last** from `index.html`, it overrides Bootstrap + the legacy css purely with CSS (custom properties in `:root`, a `body.pre-game` rule that hides the in-game chrome until `removeStartingScreen()` drops the class, and a `max-width: 767px` block for mobile: 40px touch targets, tooltip clamping, overflow-x fix). Restyle here, not in render code. |
