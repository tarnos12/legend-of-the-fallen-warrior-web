...
| `soulShopUI.js` | Soul Shop section at the top of the Shop tab (`#soulShop`): the player's Boss Soul balance + one card per boss unique (name/slot/area/soul price) with a Buy button, plus a ♻ Reforge button when a copy is owned (disabled when locked/unaffordable). Buy → `bossSouls.buyBossUnique`, reforge → `bossSouls.reforgeBossUnique` (ownership via `ownedUniqueIndex`). Re-rendered on Shop-tab open, on soul gain, and after buy/reforge. | `renderSoulShop` | `renderSoulShop` (Shop HUD nav onclick) |
...
| `bossSouls.js` | Boss-unique acquisition rework (2026-07, "Souls"): `SOUL_DROP` (souls per area-boss kill, 2× shiny) and `SOUL_SHOP` (one purchasable entry per boss unique: `bossName`, `areaType`, `def`, `price` scaling 3→15 by area). The guaranteed alternative to the RNG drop; logic in `systems/bossSouls.js`, UI in `ui/soulShopUI.js`. Tests: `test/boss-souls.test.js`. |
