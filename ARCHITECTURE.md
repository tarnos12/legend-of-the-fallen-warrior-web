...
| `battleCanvas.js` | **THE combat system** (`#battleCanvas` arena in index.html). Fight (`window.startBattle`) routes here: 1–5 copies of the selected monster charge the hero and bounce-attack round-robin (old per-turn rate; stunned enemies skip their turn); hero auto-attacks per its `weaponBehavior` profile (sword fast+crit+parry, axe cleave, mace heavy+stun, bow pierce w/ falloff, staff splash) and auto-casts the strongest affordable spell. Secondary cleave/pierce/splash damage derives from the primary roll (no lifesteal/mastery multiplication). All rules come from `systems/battle.js` exports; each enemy killed grants the classic per-kill rewards + full-heal cadence. Only enemy hp pools are local clones. setTimeout sim loop with fixed 50 ms substeps (rAF pauses in hidden tabs). | — | wraps `startBattle`; dev-only `__battleCanvasDebug` (incl. `pump(secs)` + `giveWeapon`) |
