...
// play per weapon class from a fresh level-1 character and prints a tuning
// table — time-in, kills, highest wave, level, gold, deaths. Run it after any
// combat/progression tuning change and compare classes against each other.
...
describe(`balance report (${SIM_MINUTES} simulated minutes per weapon)`, () => {
    it('prints the class comparison table', async () => {
        const rows = [];
        for (const subType of ['fists', 'sword', 'axe', 'mace', 'staff', 'ranged']) {
            rows.push(await simulate(subType));
        }
        const pad = (v, n) => String(v).padEnd(n);
        let out = '\n' + pad('WEAPON', 8) + pad('KILLS', 7) + pad('WAVE', 6);
        out += pad('LEVEL', 7) + pad('GOLD', 7) + 'DEATHS\n';
        for (const r of rows) {
            out +=
                pad(r.weapon, 8) +
                pad(r.kills, 7) +
